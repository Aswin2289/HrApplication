...
function AllPendingLeaveRequest() {
  ...
  const {
    rejectPendingLeaveAdmin,
    acceptPendingLeaveAdmin,
    deletePendingLeaveAdmin,
  ...
  const fetchLeave = async () => {
    ...
    try {
      const response = await axiosInstance.get("/leave/admin/pendingLeave", {
        params: {
          page: currentPage,
          size: rowsPerPage,
          sortBy,
      ...
      setError(error.response?.data?.message || "Failed to fetch leave");
    } finally {
      setIsLoading(false);
    }
  };
...
export default AllPendingLeaveRequest;
